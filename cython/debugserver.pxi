cdef extern from "libimobiledevice/debugserver.h":
    ...
    #TO-DO: debugserver_error_t debugserver_client_start_service(idevice_t device, debugserver_client_t * client, const char* label)
...
cdef class DebugServerClient(BaseService):
    ...
    # return response = None if error occurs
    def handle_response(self, response, reply=False):
        if not response:
            result = None
        ...
        with DebugServerCommand("OK") as reply_ok:
            response = self.send_command(reply_ok)
